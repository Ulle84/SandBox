...
- tidyPres defensiver programmieren - gucken ob zu löschende Zeichen auch wirklich Leerzeichen sind
...
function tidyPres() {
  ...
  for (var i = 0; i < pres.length; i++) {
    ...
    var lastSpace = lines[lines.length - 1].lastIndexOf(" ") + 3;
    var newText = "";
    for (var j = 0; j < lines.length - 1; j++) {
      newText = newText.concat(lines[j].slice(lastSpace));
      newText = newText.concat("\n");
    }
    pres[i].textContent = newText;
  }
}
